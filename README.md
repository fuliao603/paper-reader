...
Paper Reader 是一个本地运行的 PDF 学术阅读与翻译桌面工具，面向英文论文、教材章节、课程讲义、技术文档、扫描件和截图型资料。它把 PDF 阅读、文献库、滑词翻译、区域 OCR、图解翻译、对照翻译、批注高亮、笔记和历史数据管理放在同一个桌面应用里，方便在阅读过程中持续积累翻译结果和笔记。
...
浏览器模式主要用于前后端分开调试。可参考 `.env.example` 在项目根目录配置 `.env`：
...
- tesseract.js：本地 OCR。
- Express：本地翻译接口。
- OpenAI SDK：连接 DeepSeek、OpenRouter 或 OpenAI-compatible 接口。
