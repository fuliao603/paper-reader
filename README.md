...
- 支持左侧目录面板，优先读取 PDF 内置目录；没有内置目录时，会识别目录页或正文标题结构。
- 自动生成的目录支持论文、书籍和教材常见一级/二级结构，点击目录项可跳转到对应页。
...
- 每篇 PDF 独立保存翻译历史、笔记、批注、书签和自动目录数据。
...
- 右侧“书签”栏可为当前页添加主题书签，支持跳转、单条或批量删除、清空，并随 PDF 保存。

### 历史笔记管理

左侧“历史笔记管理”负责翻译历史、笔记、批注和书签数据的统一导入、导出和整理。

- 支持导出或导入当前 PDF 的翻译历史、笔记、批注和书签。
- 支持批量导入多个 `.paperreader.json` 备份文件并合并去重。
- 支持按 PDF 自选导出翻译历史、笔记、批注、书签或完整备份。
...
- 导入数据采用合并去重策略，不会默认覆盖已有翻译历史、笔记、批注和书签。
